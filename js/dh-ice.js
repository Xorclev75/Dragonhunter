document.addEventListener("DOMContentLoaded", () => {
		...
		function dragonAttack() {			
			let drgAtk = Math.floor(Math.random() * 40) + 10;
		...
		function updateHP() {
			...
			if (enemyHP && playerHP) {
				enemyHP.innerText = `Ice Dragon: ${eHP}/${eMaxHP}`;
		...
		function drgDefeat() {
			if (eHP <= 0) {
				...
				showModal("You defeated the Ice Dragon!  HP, Attack and Defense go up by 1 Level!");								
			}
		}

		function heroDefeat() {
			if (hHP <= 0) {
				showModal("You were defeated by the Ice Dragon...");
			}
		}
		...
		if (Potions > 0) {
			...
			hHP = Math.min(hHP + 70, hMaxHP); // Heal the hero, but don't exceed max HP
			updateHP();
			disableButtons(); // Disable buttons during action
			setTimeout(dragonAttack, 1000);
		} else {
