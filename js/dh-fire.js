document.addEventListener("DOMContentLoaded", () => {
		...
		function dragonAttack() {			
			let drgAtk = Math.floor(Math.random() * 30) + 10;
		...
		function updateHP() {
			...
			if (enemyHP && playerHP) {
				enemyHP.innerText = `Fire Dragon: ${eHP}/${eMaxHP}`;
		...
		function drgDefeat() {
			if (eHP <= 0) {
				let fdrgDefeated = true;
				localStorage.setItem('fdrgDefeated', fdrgDefeated);
				console.log("fdrgDefeated = " + fdrgDefeated);
				showModal("You defeated the Fire Dragon!  HP, Attack and Defense go up by 1 Level!");								
			}
		}

		function heroDefeat() {
			if (hHP <= 0) {
				showModal("You were defeated by the Fire Dragon...");
			}
		}
		...
		defendButton.addEventListener("click", function () {
			...
			let randomNumber = Math.floor(Math.random() * 44) + 30;
			if(rdmNumber > 45) {
			eHP = Math.max(eHP - randomNumber, 0); // Ensure HP doesn't go below 0
			updateHP();
		...
		if (Potions > 0) {
			...
			hHP = Math.min(hHP + 60, hMaxHP); // Heal the hero, but don't exceed max HP
			updateHP();
			disableButtons(); // Disable buttons during action
			setTimeout(dragonAttack, 1000);
		} else {
